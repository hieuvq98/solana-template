...
async fn set_launchpad_data_success(){
  ...
  let account_root = get_account(0);
  let account_1 = get_account(1);
  let account_5 = get_account(5);
  let account_7 = get_account(7);
  let account_8 = get_account(8);
  let coin98_mint_account = get_token_account_named(TokenName::C98);
  create_token_mint(&mut context, &coin98_mint_account, 6, &account_root.pubkey(), Option::None).await;

  let launchpad_path: Vec<u8> = (0..10).map(|_| { rand::random::<u8>() }).collect();

  let create_launchpad_ix = client::create_launchpad_instruction(
    &account_root.pubkey(),
    &account_1.pubkey(),
    launchpad_path.clone(),
    &coin98_mint_account.pubkey(),
    2000u64, // protocol fee
    10u64 // sharing fee
  );


  let whitelist_tree = create_whilelist_tree(
    &[
      account_5.pubkey(),
      account_7.pubkey(),
      account_8.pubkey(),
  ...
  let (launchpad_address, _) = client::find_launchpad_address(launchpad_path);
  let current_time = get_current_timestamp();
  let set_launchpad_ix = client::set_launchpad_instruction(
    &account_1.pubkey(),
    &launchpad_address,
    1000000,
    1,
    ...
    current_time + 604800,
    current_time + 864000,
    Some(whitelist_tree.root.hash.try_into().unwrap()),
  );

  airdrop_lamport(&mut context, &account_root.pubkey(), 2526480).await;
  let payer = get_payer(&context);
  process_transaction(
    &mut context,
    &payer,
    &[create_launchpad_ix, set_launchpad_ix],
    &[&payer, &account_root, &account_1],
  ).await;
}
