...
async fn user_redeem_before_schedule(){
    println!(">>>>>>>>>> Fail: user redeem before schedule <<<<<<<<<<<<<<<");
    let mut context = c98_starship_program_test().start_with_context().await;
    ...
    let user1_register_data = register_data_instruction(&launchpad_address, &user1.pubkey(), &user1_global_profile, &user1_local_profile, 0, user1_proofs);
    let result0 = process_transaction_with_error(&mut context, &Vec::from([user1_register_data]), &Vec::from([&user1])).await;
    match result0 {
      Ok(_) => assert!(result0.is_err()),
      Err(_) => {},
    };
    ...
    airdrop(&mut context, &launchpad_signer, 10_000_000_000).await.unwrap();
    let user1_redeem_token_by_sol =  redeem_by_sol_data_instruction(&user1.pubkey(), &launchpad_address, &launchpad_signer, &user1_global_profile, &user1_local_profile, &user1_c98_token_account, &launchpad_c98_token_account, 20);
    let result1 =  process_transaction_with_error(&mut context, &Vec::from([user1_redeem_token_by_sol]), &Vec::from([&user1])).await;
    match result1 {
      Ok(_) => assert!(result1.is_err()),
      Err(_) => {},
    };
...
#[tokio::test]
async fn user_claim_wrong_option(){
    println!(">>>>>>>>>> FAIL: user claim wrong option <<<<<<<<<<<<<<<");
        let mut context = c98_starship_program_test().start_with_context().await;
    ...
    // user claim less than min claim
    let user1_redeem_token_by_sol0 =  redeem_by_sol_data_instruction(&user1.pubkey(), &launchpad_address, &launchpad_signer, &user1_global_profile, &user1_local_profile, &user1_c98_token_account, &launchpad_c98_token_account, 5);
    let result0 = process_transaction_with_error(&mut context, &Vec::from([user1_redeem_token_by_sol0]), &Vec::from([&user1])).await;
    match result0 {
      Ok(_) => assert!(result0.is_err()),
      Err(_) => {},
    };
    // user claim more than max claim
    let user1_redeem_token_by_sol1 =  redeem_by_sol_data_instruction(&user1.pubkey(), &launchpad_address, &launchpad_signer, &user1_global_profile, &user1_local_profile, &user1_c98_token_account, &launchpad_c98_token_account, 200);
    let result1 = process_transaction_with_error(&mut context, &Vec::from([user1_redeem_token_by_sol1]), &Vec::from([&user1])).await;
    match result1 {
      Ok(_) => assert!(result1.is_err()),
      Err(_) => {},
    };
...
#[tokio::test]
async fn user_send_wrong_token(){
    println!(">>>>>>>>>> Fail: user send wrong token when claim <<<<<<<<<<<<<<<");
    let mut context = c98_starship_program_test().start_with_context().await;
    ...
    create_mint(&mut context, &c98_mint, &payer_wallet.pubkey(), None).await.unwrap();
    create_mint(&mut context, &cusd_mint, &payer_wallet.pubkey(), None).await.unwrap();
    create_mint(&mut context, &usdt_mint, &payer_wallet.pubkey(), None).await.unwrap();
    let user1_c98_token_account = create_associated_token_account(&mut context, &user1.pubkey(), &c98_mint.pubkey()).await.unwrap();
    let user1_cusd_token_account = create_associated_token_account(&mut context, &user1.pubkey(), &cusd_mint.pubkey()).await.unwrap();
    let user1_usdt_token_account = create_associated_token_account(&mut context, &user1.pubkey(), &usdt_mint.pubkey()).await.unwrap();
    mint_tokens(&mut context, &cusd_mint.pubkey(), &user1_cusd_token_account, 1_000_000_000_000, &payer_wallet.pubkey(), Some(&payer_wallet)).await.unwrap();
    mint_tokens(&mut context, &usdt_mint.pubkey(), &user1_usdt_token_account, 1_000_000_000_000, &payer_wallet.pubkey(), Some(&payer_wallet)).await.unwrap();
      //create vault
    let (launchpad_address,_) = find_launchpad_address(feed_path);
...
#[tokio::test]
async fn reach_to_limit_sale(){
   println!(">>>>>>>>>> FAIL: user claim when reach to limit sale <<<<<<<<<<<<<<<");
    let mut context = c98_starship_program_test().start_with_context().await;
    ...
    create_mint(&mut context, &c98_mint, &payer_wallet.pubkey(), None).await.unwrap();
    let user1_c98_token_account = create_associated_token_account(&mut context, &user1.pubkey(), &c98_mint.pubkey()).await.unwrap();
    let user2_c98_token_account = create_associated_token_account(&mut context, &user2.pubkey(), &c98_mint.pubkey()).await.unwrap();
      //create vault
    let (launchpad_address,_) = find_launchpad_address(feed_path);
    ...
    let time = SystemTime::now()
        ...
        .unwrap()
        .as_secs() as i64;
    let set_launchpad_data =  set_launchpad_data_instruction(&payer_wallet.pubkey(), &launchpad_address, 1000, 1, 10, 100, 100, time + 10, time + 100, time + 1000, time + 2000, Some(merkle_root));
    ...
    let user1_redeem_token_by_sol =  redeem_by_sol_data_instruction(&user1.pubkey(), &launchpad_address, &launchpad_signer, &user1_global_profile, &user1_local_profile, &user1_c98_token_account, &launchpad_c98_token_account, 100);
    process_transaction(&mut context, &Vec::from([user1_redeem_token_by_sol]), &Vec::from([&user1])).await.unwrap();

    let user2_redeem_token_by_sol =  redeem_by_sol_data_instruction(&user2.pubkey(), &launchpad_address, &launchpad_signer, &user2_global_profile, &user2_local_profile, &user2_c98_token_account, &launchpad_c98_token_account, 100);
    let result = process_transaction_with_error(&mut context, &Vec::from([user2_redeem_token_by_sol]), &Vec::from([&user2])).await;
    match result {
      Ok(_) => assert!(result.is_err()),
      Err(_) => {},
    };
...
async fn stranger_withdraw_token(){
  println!(">>>>>>>>>> FAIL: user not admin withdraw sol or token <<<<<<<<<<<<<<<");
  let mut context = c98_starship_program_test().start_with_context().await;
...
#[tokio::test]
async fn register_redeem_before_active(){
    println!(">>>>>>>>>> FAIL: user register and redeem before active <<<<<<<<<<<<<<<");
    let mut context = c98_starship_program_test().start_with_context().await;
...
async fn user_unban_blacklist_register_claim(){
    println!(">>>>>>>>>> FAIL: user unban from blacklist redeem using sol <<<<<<<<<<<<<<<");
    let mut context = c98_starship_program_test().start_with_context().await;
    ...
    let user1_redeem_token_by_sol1 =  redeem_by_sol_data_instruction(&user1.pubkey(), &launchpad_address, &launchpad_signer, &user1_global_profile, &user1_local_profile, &user1_c98_token_account, &launchpad_c98_token_account, 20);
    let result0 =process_transaction_with_error(&mut context, &Vec::from([user1_redeem_token_by_sol1]), &Vec::from([&user1])).await;
    match result0 {
      Ok(_) => assert!(result0.is_err()),
      Err(_) => {},
    };
